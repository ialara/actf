...
days = [1,     10,     3,     4,      2,     9]
ugs = ['IPUG','IPUG', 'FLUG', 'MQT', 'FLUG', 'MQT']
...
#  ('FLUG', 2) <- [4],
#  ('FLUG', 3) <- [2]]
## so UG-date priority of pilot id should be: [3, 5, 0, 1, 4, 2]
...
def test_students_prioritized_by_ug(my_students, my_scheduler, ug_priorities):
    expected_ids = [3, 5, 0, 1, 2, 4] # Matches in UG are left in order they were passed
...
def test_scheduling_philosophy_upgrade_then_date(my_students, my_scheduler, ug_priorities):
    expected_ids = [3, 5, 0, 1, 4, 2] # See above
    prioritized = my_scheduler.prioritize_students_by_flight_date(my_students)
